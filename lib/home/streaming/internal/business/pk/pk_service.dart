...
class PKService implements PKServiceInterface {
  ...
  Future<ZegoMixerStartResult> updatePKMixTask() async {
    ...
    if (task == null) {
      final mixStreamID = generateMixerStreamID();
  ...
  List<ZegoMixerInput> getMixVideoInputs(
    ...
    if (streamList.length == 2) {
      for (var i = 0; i < 2; i++) {
        ...
        const top = 0.0;
        final width = videoConfig.width / 2.0;
        final height = videoConfig.height.toDouble();
      ...
      for (var i = 0; i < 3; i++) {
        ...
        final top = i == 2.0 ? (videoConfig.height / 2.0) : 0.0;
        final width = videoConfig.width / 2.0;
        final right = i == 0 ? width : videoConfig.width.toDouble();
        final bottom =
            i == 1 ? (videoConfig.height / 2.0) : videoConfig.height.toDouble();
      ...
      var lastLeft = 0.0;
      final height = videoConfig.height / 2.0;
      for (var i = 0; i < 5; i++) {
        if (i == 2) {
          lastLeft = 0.0;
        }
  ...
  String anotherHostStreamID() {
    return hostStreamIDFormat(pkUser?.roomID ?? '', pkUser?.userID ?? '');
  }
